    ...
    def post(self):
        ...
        if user.role != "customer":
            return make_response(
                jsonify(message="Only customers can submit reviews"), 403
            )
        ...
        order = Order.query.filter_by(
            customer_id=identity, product_id=product_id, order_status="completed"
        ).first()
        if not order:
            return make_response(
                jsonify(
...
class ProductReview(Resource):
    ...
    def delete(self, review_id):
        ...
        if review.customer_id != identity:
            return make_response(
                jsonify(error="You are not authorized to delete this review"), 403
            )
...
class UpdateProduct(Resource):
    ...
    def put(self, product_id):
        ...
        product = Product.query.filter_by(
            id=product_id, farmer_id=current_farmer_id
        ).first()
        if not product:
...
class DeleteOrder(Resource):
    ...
    def delete(self, order_id):
        ...
        # Retrieve the order
        order = Order.query.filter_by(
            id=order_id, customer_id=current_customer_id
        ).first()
...
@app.route("/api/orders/check-purchase/<int:product_id>", methods=["GET"])
@jwt_required()
